...
#Perform batch adjustment of markers in mass cytometry data 
#Further details in Schuyler, et al. 2019 (https://10.3389/fimmu.2019.02367)

print("Start 03_BatchAdjust.R")

rm(list = ls(all = TRUE))
.libPaths(c( .libPaths(), "~/R/4.1.1"))

library(flowCore) # to read.FCS
...
get_summaries_per_channel <- function(basedir, cols_to_use, batchKeyword="Plate", anchorKeyword = "Sample2"){
   ...
   for(fname in filelist){
      ...
      vals <- apply(fce[,tf2use], MARGIN=2, FUN="var"); # or var
